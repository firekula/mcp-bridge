...
Editor.Panel.extend({
    ...
    injectMcpConfig(clientId, els) {
        ...
        Editor.Ipc.sendToMain("mcp-bridge:mcp-inject-client", clientId, (err, log) => {
            if (err) {
                this.renderLog({ type: "error", time: new Date().toLocaleTimeString(), content: "配置失败: " + err.message });
            } else {
                this.renderLog({ type: "success", time: new Date().toLocaleTimeString(), content: "MCP 配置结果:\n" + log });
            }
            // 回到主页以便查看日志
            if (els.tabMain) {
